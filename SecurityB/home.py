...
def sell_home(_pk,_price):
    transact = contract.functions.add_home(_pk,_price).transact({'from': web3.eth.defalutAccount}) # transaction
    result = contract.call().add_home(_pk,_price)
    result = str(result)
