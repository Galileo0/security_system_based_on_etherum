...
def add_res(_email,_pk,_res_add):
    transact = contract.functions.add_resdent(_email,_pk,_res_add).transact({'from': web3.eth.defalutAccount}) # transaction
    result = contract.call().add_resdent(_email,_pk,_res_add)
    result = str(result)